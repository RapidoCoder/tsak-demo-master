...
public class FFManager {
	...
	public FFManager(Twitter twtr, DDManager ddm, CrManager crm, LimitsManager lmg) {
	...
	public ResponseList<User> dumpFFToFile(String tuser, subCmdUpVector sbv)
		...
		try {
			...
			if (sbv == subCmdUpVector.FLWRZ_DUMP_BY_ID || sbv == subCmdUpVector.FLWRZ_DUMP_BY_SCREEN_NAME) {
				...
				do {
					...
					int lmts[] = cRManager.rateLimitAnalyzer(twitter, lManager,LimitsEndPointsVector.FOLLOWERS_IDS);
					...
					if (availableCalls == 0){
						throw new TsakException("Call not available, please try later.");
					}
			...
			} else if (sbv == subCmdUpVector.FRNDZ_DUMP_BY_ID || sbv == subCmdUpVector.FRNDZ_DUMP_BY_SCREEN_NAME) {
				...
				do {
					...
					if (sbv == subCmdUpVector.FRNDZ_DUMP_BY_ID) {
						friendsIds = twitter.getFriendsIDs(Long.parseLong(tuser) , cursor);
					}else {
						
						friendsIds = twitter.getFriendsIDs(String.valueOf(tuser), cursor);
					}
					
					cRManager.DisplayInfoMessage(
							"[INFO]: Checking Rate Limits Availibity.", true);

					int lmts[] = cRManager.rateLimitAnalyzer(twitter, lManager,LimitsEndPointsVector.FRIENDS_IDS);
			...
			if (te.getErrorCode() == 404) {
				cRManager.DisplayErrorMessage("[ERROR]: Target User Doesn't Exist.", true);
			} else {
				cRManager.DisplayErrorMessage("[ERROR]: Unable to dump followers." + te.getMessage(), true);
			}
